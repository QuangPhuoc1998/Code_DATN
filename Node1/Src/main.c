...
int main(void)
{
  ...
  MX_USART1_UART_Init();
	...
	//RTC_SetTime(11,35,0,2,11,20);
	lcd_init();
	HAL_Delay(100);
	lcd_send_string("Test");
	//QMC5883_Set();
	//HAL_UART_Transmit(&huart1, (uint8_t *)"HELLO", 1, 1000);
  while (1)
  {
		RTC_GetTime();
		LM75_GetValue();
		...
		if(HAL_GPIO_ReadPin(GPIOA, GPIO_PIN_4) == 0)
		{
			...
			sprintf(buffer_to_send,"1,%d,%d,%d",temp, value, HX711_raw);
...
void GY30_GetValue(void)
{
	...
	value = ((GY30.Conti_H_Mode1<<8)+GY30.Conti_H_Mode2)/1.2;
}
/*--- QMC5883 set ---*/
void QMC5883_Set(void)
{
	I2C_Buffer[0] = 0x0B;
	I2C_Buffer[1] = 0x01;
	I2C_WriteBuffer(hi2c1,(uint16_t)QMC5883_ADRESS, 2);
	HAL_Delay(100);
}
/*--- QMC5883 get ---*/
void QMC5883_Get(void)
{
	I2C_Buffer[0] = 0x00;
	I2C_WriteBuffer(hi2c1,(uint16_t)QMC5883_ADRESS, 1);
	while(HAL_I2C_GetState(&hi2c1) != HAL_I2C_STATE_READY);
	I2C_ReadBuffer(hi2c1, (uint16_t)QMC5883_ADRESS, 7);
	
	QMC5883.X_LSB  = I2C_Buffer[0];
	QMC5883.X_MSB  = I2C_Buffer[1];
	QMC5883.Y_LSB  = I2C_Buffer[2];
	QMC5883.Y_MSB  = I2C_Buffer[3];
	QMC5883.Z_LSB  = I2C_Buffer[4];
	QMC5883.Z_MSB  = I2C_Buffer[5];
